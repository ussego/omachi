...
export function graphTransition(reduce: boolean | null, extras?: Transition): Transition {
	if (reduce) {
		return { duration: 0 };
	}
...
export function toneClass(palette: GraphPalette | undefined, role: "primary" | "secondary" | "idle" | "empty") {
	if (role === "empty") {
		return "text-graph-frame";
	}
