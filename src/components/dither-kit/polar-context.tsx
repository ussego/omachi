...
import { type AreaVariant, type ChartConfig, type ChartType, type Margins, useRevision } from "./chart-context";
...
export function usePolarPart(part: string, kind: "pie" | "radar") {
	...
	if (ctx.chartType !== kind) {
		throw new Error(`<${part} /> is not valid inside ${ROOT_OF[ctx.chartType]} — it belongs in ${ROOT_OF[kind]}.`);
	}
	return ctx;
}
...
export function usePolarController({
	...
	const variantOf = useCallback((key: string) => variants[key] ?? variants["*"] ?? "gradient", [variants]);
