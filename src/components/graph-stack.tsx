...
function paintRow(segments: StackSegment[], ticks: number, glyphs: readonly string[], accentLabel?: string): Painted[] {
			...
			<GraphBody className="flex flex-col gap-6">
				...
				<ul className="flex flex-wrap gap-x-4 gap-y-1" role="list">
					{legend.map((label, index) => {
						...
						return (
							<li className="flex items-center gap-2" key={label} style={seriesDim(palette, highlighted)}>
								<span aria-hidden="true" className={seriesClass(palette, index)}>
									{glyph}
								</span>
								<span className={highlighted ? "text-foreground" : "text-graph-muted"}>{label}</span>
							</li>
						);
					})}
				</ul>
