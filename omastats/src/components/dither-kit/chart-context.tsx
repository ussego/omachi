...
export function useChartController({
	...
	const { bands, max, min } = useMemo(() => computeBands(data, configKeys, stackType), [data, configKeys, stackType]);
	...
	const indexAtX = useCallback(
		(px: number) => (isBar ? indexAtBand(px, data.length, plotWidth) : nearestIndex(px, data.length, plotWidth)),
